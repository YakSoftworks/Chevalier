...
void ChevalierRenderer::initRenderer()
{
    ...
    globalDataManager = new GlobalDescriptorDataManager();
...
void ChevalierRenderer::createDescriptorPool() {
    ...
    //Specify the quantity - Attempting 10 per Frame in Flight
    poolSizes[0].descriptorCount = 10 * static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);

    //Same case 
    poolSizes[1].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[1].descriptorCount = 10 * static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);

    //Add in Storage Buffers
    poolSizes[2].type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    poolSizes[2].descriptorCount = 10 * static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
...
void ChevalierRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    for (size_t i = 0; i < renderObjects.size(); i++) {
        ...
        PerObjectData* objectMatrixArr = static_cast<PerObjectData*>(globalDataManager->objectBufferMappedMemory[currentFrame]);
        objectMatrixArr[i].modelMat = renderObjects[i]->getModelMatrix();


        renderObjects[i]->RecordObjectDraw(commandBuffer);
    }
